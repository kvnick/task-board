...
const TaskDetailPage = (props) => {
  ...
  const handleCancel = useCallback(() => history.push(normalizedRoutes.tasks), [
    history,
  ])
    ...
    <Page>
      ...
      >
            ...
            <Tab
              label={
                <Badge
                  ...
                  color="primary"
                  badgeContent={historyLength > 0 ? historyLength : null}
                >
                  Task History
                </Badge>
              }
        ...
        <TabPanel value={tabValue} index={0}>
          <Card>
            <CardHeader
              title="Task info"
              subheader={
                task && task.createdDate && `${formatDate(task.createdDate)}`
              }
              avatar={
                task &&
                task.user && (
        ...
        <TabPanel value={tabValue} index={1}>
          {task ? <TaskHistoryContainer history={task.history} /> : <Loading />}
        </TabPanel>
      </Container>
    </Page>
  )
